...
def clear_grid_output_dir():
    ...
    """
    logging.info('Clearing User Grid output directory... : %s/%s',
                 GRID_USER_HOME, GRID_SITE_SONAR_OUTPUT_DIR)
    command = 'alien.py rm -rf {}/{}'.format(
        GRID_USER_HOME, GRID_SITE_SONAR_OUTPUT_DIR)
...
def job_submission(jdl_name):
    ...
    if success_flag:
        ...
        for site in site_details:
            ...
            logging.info('Submitting %s jobs to the Grid site %s',
                         str(num_jobs - 1), sitename)
            for i in range(1, num_jobs):
                output_dir = get_grid_output_dir(
                    GRID_SITE_SONAR_OUTPUT_DIR, site['normalized_name'], i)
                ...
                command = 'alien.py submit {} {} {} {}'.format(
                    job_path, site_sonar_dir, sitename, output_dir)
...
def stage_jobs_to_grid(dirname):
    """
    Upload the files to the GRID HOME

    Args:
        dirname (str): Name of the directory that contain the files to be uploaded
    ...
    # Return code 2 is given if file does not exist. Ignore that error
    if p.returncode != 0 and p.returncode != 2:
        raise CalledProcessError(p.returncode, p.args)

    for file in os.listdir(dirname):
        command = 'alien.py cp -T 32 file:{}/{} alien:{}/{}@disk:1'.format(
            dirname, file, GRID_DIR, file)
        with Popen(shlex.split(command), stdout=PIPE, bufsize=1, universal_newlines=True) as p:
            logging.info('Staging jobs ./%s/%s to %s/%s',
                         dirname, file, GRID_DIR, file)
            for line in p.stdout:
                logging.info('> %s ', line.rstrip())
        if p.returncode != 0:
